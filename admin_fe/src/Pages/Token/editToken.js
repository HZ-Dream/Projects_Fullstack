...
const EditToken = () => {
    ...
    useEffect(() => {
        ...
        fetchDataFromApi(`/api/token/getItem/${id}`).then((res) => {
            setFormField(res);
        });
    }, []);
    ...
    const updateToken = (e) => {
        ...
        try {
            editData(`/api/token/updateToken/${id}`, formField)
                .then((res) => {
                    context.handleClickVariant('Update token success!', 'success');
                    setIsLoad(false);
                })
                .catch((err) => {
                    setIsLoad(false);
    ...
    return (
        <section className="right-content w-100 createQuiz">
            <form onSubmit={updateToken} className="form">
                <div className="row">
